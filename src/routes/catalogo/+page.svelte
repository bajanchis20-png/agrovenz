<script lang="ts">
    ...
    // Estructura adaptada con variantes (presentaciones)
    interface Variante {
        id: number;
        volumen: string;
        price: number;
    ...
    const productosCatalogo: ProductoAgrupado[] = [
        ...
        { tituloBase: "Dectomec Gold - ivermectina 3,15%", cat: "Medicina Veterinaria", subcat: "Antiparasitarios", variantes: [
            { id: 40, volumen: "100 ml", price: 10.96, images: ["/dectomec.jpeg"], desc: "Ivermectina de alta concentración al 3,15%." },
            { id: 41, volumen: "500 ml", price: 38.00, images: ["/dectomec.jpeg"], desc: "Presentación de 500 ml de ivermectina 3,15%." }
        ]},
    ...
    // Estado para guardar la variante seleccionada de cada tarjeta (clave: índice del producto agrupado, valor: índice de la variante)
    let seleccionVariantes = $state<Record<number, number>>({});
    let indicesImagenes = $state<Record<string, number>>({});
    ...
    <header class="sticky top-0 bg-stone-50/95 backdrop-blur-md z-40 pb-3 pt-2 border-b border-stone-200 mb-4 sm:mb-6">
        <div class="flex flex-col sm:flex-row gap-2.5 sm:gap-4 items-center justify-between">
            ...
            <div class="relative w-full sm:w-96">
                <div class="relative flex items-center">
                    ...
                    {#if busqueda}
                        <button on:click={() => busqueda = ""} class="absolute right-3 p-1 text-stone-400 hover:text-stone-600"><Icon icon="mdi:close-circle" class="text-base"/></button>
                    {/if}
                </div>
                ...
                {#if resultadosPredictivos.length > 0}
                    <div class="absolute top-full left-0 right-0 mt-1.5 bg-white rounded-2xl shadow-xl border border-stone-100 overflow-hidden z-50 max-h-60 overflow-y-auto">
                        {#each resultadosPredictivos as r}
                            <button on:click={() => { busqueda = r.tituloBase; busqueda = ""; }} class="w-full text-left px-4 py-3 hover:bg-stone-50 border-b border-stone-50 flex items-center justify-between gap-2 touch-manipulation">
                                <div>
        ...
        <div class="hidden sm:flex gap-2 mt-4 overflow-x-auto pb-1 scrollbar-none">
            {#each macroCategorias as macro}
                <button on:click={() => { macroFiltro = macro; subFiltro = "Todos"; }} class="px-4 py-2 rounded-xl text-xs font-bold uppercase tracking-wider transition-all border {macroFiltro === macro ? 'bg-emerald-800 text-white border-emerald-800 shadow-xs' : 'bg-white text-stone-700 border-stone-200 hover:border-emerald-800'}">{macro}</button>
            {/each}
        </div>
    </header>
    ...
    <div class="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2.5 sm:gap-6">
        {#each productosFiltrados as p, index (p.tituloBase)}
            ...
            <div class="bg-white p-2.5 sm:p-5 rounded-2xl sm:rounded-3xl border border-stone-100 shadow-xs hover:shadow-xl transition-all flex flex-col h-full group">
                ...
                <!-- SELECTOR DE PRESENTACIONES (Si tiene más de una variante) -->
                    ...
                    <div class="mb-2">
                        ...
                        <div class="flex flex-wrap gap-1">
                            {#each p.variantes as v, vIdx}
                                <button on:click={() => cambiarVariante(index, vIdx)} class={`px-2 py-1 rounded-lg text-[9px] font-bold transition-all touch-manipulation ${(seleccionVariantes[index] || 0) === vIdx ? 'bg-emerald-800 text-white shadow-xs' : 'bg-stone-100 text-stone-700 hover:bg-stone-200'}`}>
                                    {v.volumen}
                                </button>
                            {/each}
                        </div>
    ...
    {#if carritoAbierto}
        ...
        <div class="fixed top-0 right-0 h-full w-full sm:w-[420px] bg-white shadow-2xl z-50 p-4 sm:p-6 flex flex-col transition-transform transform translate-x-0">
            <div class="flex justify-between items-center mb-4 sm:mb-6 pb-2 border-b border-stone-100">
                ...
                <button on:click={() => carritoAbierto = false} class="p-2 rounded-full hover:bg-stone-100 active:bg-stone-200 touch-manipulation"><Icon class="text-stone-600 text-xl" icon="mdi:close"/></button>
            </div>
            ...
            {:else}
                    ...
                    {#each carrito as item}
                        <div class="flex justify-between items-center bg-stone-50 p-3 rounded-2xl border border-stone-100 gap-2">
                            ...
                            <div class="flex gap-2 items-center">
                                <button on:click={() => cambiarCantidad(item.id, -1)} class="w-7 h-7 flex items-center justify-center rounded-lg bg-white border border-stone-200 font-bold text-xs">-</button>
                                <span class="font-bold w-4 text-center text-xs text-stone-800">{item.cantidad}</span>
                                <button on:click={() => cambiarCantidad(item.id, 1)} class="w-7 h-7 flex items-center justify-center rounded-lg bg-white border border-stone-200 font-bold text-xs">+</button>
                                <button on:click={() => eliminar(item.id)} class="p-1 hover:bg-red-50 rounded-lg transition-colors"><Icon class="text-red-500 text-lg" icon="mdi:trash-can-outline"/></button>
                            </div>
                        </div>
                    {/each}
                </div>
                ...
                <button on:click={finalizarCompra} class="w-full bg-emerald-800 text-white py-3.5 sm:py-4 rounded-2xl font-black uppercase text-xs sm:text-sm hover:bg-emerald-900 transition-all tracking-wider shadow-md flex items-center justify-center gap-2">
