        ...
        <div class="grid lg:grid-cols-2 gap-8 lg:gap-16 items-center">
            <div class="space-y-4 sm:space-y-6 text-center lg:text-left">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 py-2 text-left">
                    <div class="flex items-center space-x-3 p-3 rounded-xl bg-stone-50 border border-stone-100 hover:border-emerald-600/30 transition-all">
                        <div class="flex-shrink-0 w-8 h-8 rounded-lg bg-emerald-800/10 flex items-center justify-center text-emerald-800">
    ...
    <div class="max-w-7xl mx-auto px-3 xs:px-4 sm:px-6 lg:px-8">
        <div class="grid lg:grid-cols-2 gap-8 lg:gap-16 items-center">
            <div class="space-y-4 sm:space-y-6 text-center lg:text-left">
                ...
                <p class="text-stone-600 text-sm sm:text-lg">Diseñamos planes de inversión a la medida de tu realidad productiva.</p>
                <div class="flex flex-col sm:flex-row justify-center lg:justify-start gap-4 pt-2">
                    <a href="{waLink}Hola, deseo ver catálogo de fincas." target="_blank" class="w-full sm:w-auto px-6 xs:px-8 py-3.5 xs:py-4 bg-stone-900 text-white hover:bg-emerald-800 rounded-xl text-[9px] xs:text-[10px] lg:text-[11px] font-bold uppercase tracking-[0.155em] lg:tracking-[0.2em] transition-all text-center">Ver Fincas</a>
                </div>
            </div>
            <div class="relative h-[260px] xs:h-[320px] sm:h-[400px] lg:h-[500px] rounded-2xl lg:rounded-[2rem] overflow-hidden shadow-2xl">
...
<section class="py-12 sm:py-16 lg:py-24 bg-stone-50 relative">
    <div class="max-w-7xl mx-auto px-3 xs:px-4 sm:px-6 lg:px-8">
        <div class="grid lg:grid-cols-2 gap-8 lg:gap-16 items-center">
            <div class="space-y-4 sm:space-y-6 text-center lg:text-left order-2 lg:order-1">
                ...
                <p class="text-stone-600 text-sm sm:text-lg">Geoplasma exclusivo.</p>
                <div class="flex flex-col sm:flex-row justify-center lg:justify-start gap-4 pt-2">
                    <a href="{waLink}Hola, deseo información sobre Plántula in vitro." target="_blank" class="w-full sm:w-auto px-6 xs:px-8 py-3.5 xs:py-4 bg-stone-900 text-white hover:bg-emerald-800 rounded-xl text-[9px] xs:text-[10px] lg:text-[11px] font-bold uppercase tracking-[0.155em] lg:tracking-[0.2em] transition-all text-center">Explorar paquetes</a>
                </div>
            </div>
            <div class="relative h-[260px] xs:h-[320px] sm:h-[400px] lg:h-[500px] rounded-2xl lg:rounded-[2rem] overflow-hidden shadow-2xl order-1 lg:order-2">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 xs:gap-6 lg:gap-8">
            {#each productos.slice(0, 4) as p}
                <div class="group bg-white p-4 rounded-2xl border border-stone-200 hover:border-emerald-800 transition-all duration-500 shadow-sm hover:shadow-xl flex flex-col justify-between">
                    <div>
                        <img src={p.image} alt={p.title} class="w-full h-44 xs:h-48 sm:h-52 object-cover rounded-xl mb-4 bg-stone-100" />
                        <h3 class="font-bold text-stone-900 mb-1 text-sm sm:text-base">{p.title}</h3>
                        <p class="text-[9px] xs:text-[10px] text-stone-400 uppercase tracking-widest mb-3">{p.cat}</p>
                    </div>
                    <span class="block font-black text-base sm:text-lg text-emerald-800 mt-2">${p.price}</span>
                </div>
            {/each}
        </div>
    </div>
    ...
    <div class="max-w-7xl mx-auto px-3 xs:px-4 sm:px-6 lg:px-8 text-center">
        ...
        <div class="grid grid-cols-2 md:grid-cols-3 gap-3 xs:gap-4 sm:gap-6">
            {#each categorias as cat}
                <a href="/catalogo" class="group flex flex-col items-center p-5 xs:p-6 sm:p-8 bg-white sm:bg-stone-50 border border-stone-200 sm:border-stone-100 rounded-2xl sm:rounded-3xl hover:border-emerald-800 transition-all hover:bg-white shadow-sm sm:shadow-none">
                    <div class="w-12 h-12 xs:w-14 xs:h-14 sm:w-16 sm:h-16 bg-stone-100 rounded-full flex items-center justify-center mb-3 xs:mb-4 sm:mb-6 group-hover:bg-emerald-100 transition-all">
                        <Icon icon={cat.icon} class="text-xl xs:text-2xl text-stone-600 group-hover:text-emerald-800" />
                    </div>
